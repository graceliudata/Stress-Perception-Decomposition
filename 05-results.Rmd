...
v_content %>% arrange(desc(severity)) %>%
  ...
  scale_color_manual(values = c("#fe9c8f","#84c1ff")) + labs(title = "Grouped by Sex")+ theme(legend.position="bottom")+ theme(axis.text.x = element_text(size=5))
...
v_content %>% arrange(desc(severity)) %>%
  ...
  scale_color_viridis(discrete=TRUE)+ labs(title = "Grouped by Household Income")+ theme(legend.position="bottom")+ theme(axis.text.x = element_text(size=5))

v_content %>% arrange(desc(severity)) %>%
  select(ID,Score,vignette,Household_income,Relationship,Loneliness) %>%
  pivot_wider(names_from = vignette, values_from = Score) %>%
  ggparcoord(columns = (5:60),scale = "globalminmax", alphaLines = 0.5, groupColumn = "Loneliness",splineFactor = 10) + 
  scale_color_viridis(discrete=TRUE)+ labs(title = "Grouped by Lonliness")+ theme(legend.position="bottom")+ theme(axis.text.x = element_text(size=5))
...
v_content %>% arrange(desc(severity)) %>%
  ...
  scale_color_viridis(discrete=TRUE)+ labs(title = "Relationship with Close Ones")+ theme(legend.position="bottom")+ theme(axis.text.x = element_text(size=5))
