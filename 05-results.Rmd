...
From mosaic plot, we can see higher severity contributes more to the higher score, that is, the vignette with higher severity yield more high scores in proportion. From the bar plot, we can see an approximate "V" shape. With lower severity(1-3), the counts of scores is in an approximate descending order(score 0 counts most, score 4 counts least). With higher severity(3-4), the counts of scores is in an approximate ascending order(score 0 counts least, score 4 counts most). The bar plot can explain the proportion pattern presents in the mosaic plot consistently.
```{r echo=FALSE}
colors = c('#EBF7E3',
          '#9BD770',
          '#66B032',
...
vcd::mosaic(severity~ score,v_content, direction = c("v","h"),
           highlighting_fill = colors, labeling_args = list(set_varnames = c(score="Subjective Stress", severity="Objective Stress")),main = "Subjective Stress v.s. Objective Stress")
